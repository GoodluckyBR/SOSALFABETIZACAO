    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex space-x-8 text-sm font-medium text-slate-600">
          <button onClick={() => scrollToSection('how-it-works-section')} className="hover:text-[#D62828] transition-colors cursor-pointer font-semibold">Como funciona</button>
          <button onClick={() => scrollToSection('literacy-map')} className="hover:text-[#D62828] transition-colors cursor-pointer font-semibold">Mapa da Alfabetização</button>
          <button onClick={() => scrollToSection('deliverables-section')} className="hover:text-[#D62828] transition-colors cursor-pointer font-semibold">O que você recebe</button>
          <button onClick={() => scrollToSection('about-professional')} className="hover:text-[#D62828] transition-colors cursor-pointer font-semibold">Sobre Tainá</button>
          <button onClick={() => scrollToSection('faq')} className="hover:text-[#D62828] transition-colors cursor-pointer font-semibold">Dúvidas</button>
        </nav>
      ...
      {isOpen && (
        <div className="lg:hidden absolute top-16 md:top-20 left-0 w-full bg-white border-b border-slate-100 shadow-xl py-6 px-4 flex flex-col space-y-4 animate-fade-in-down">
          ...
          <button onClick={() => scrollToSection('about-professional')} className="text-left py-2 font-medium text-slate-700 hover:text-[#D62828]">Sobre Tainá</button>
